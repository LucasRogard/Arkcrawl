...
import { writeFile } from 'fs/promises';
  ...
  (async () => {
    ...
    await page.goto(`https://www.pocketgamer.com/arknights/tier-list/`)

    const dico = []

    //liste des tableaux de personnages
    const tabs = await page.locator('table').filter({ hasText: 'Operator' });
    ...
    await writeFile('pocketgamer.json', JSON.stringify(dico, null, 2));
